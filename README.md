...
* **Multilingual translation comparison**: Chinese, English, and Vietnamese can be enabled independently; untranslated content is translated live with Google Translation Basic v2 and cached in the current browser.
...
All translation switches default to off. The small set of curated Chinese translations lives in `assets/translations_zh.json`; optional `question_zh` and `explanation_zh` fields inside question-bank JSON are also supported. All other translations are generated through Google Cloud Translation Basic v2 and cached locally in the browser by question and target language. See the [translation setup guide](docs/translation_backend.md) for setup and testing.
