...
* **Chinese translation comparison**: Enable it in settings; untranslated questions are translated live with Google and cached on the server.
...
The small set of curated translations lives in `assets/translations_zh.json`; optional `question_zh` and `explanation_zh` fields inside question-bank JSON are also supported. All other translations are generated through Google Cloud Translation and cached in Firestore for reuse. See the [Chinese translation backend guide](docs/translation_backend.md) for setup and testing.
